...
class S3ServiceIT {
    ...
    @Test
    void testUploadFile() {
        MockMultipartFile file = new MockMultipartFile("file", "test2.txt",
                "text/plain", "test content".getBytes());
    ...
    void testDownloadFile() {
        MockMultipartFile file = new MockMultipartFile("file", "test3.txt",
                "text/plain", "test content".getBytes());
        List<String> fileId = s3Service.uploadFile(file);
        byte[] result = s3Service.downloadFile(fileId.getFirst());
    ...
    void testDeleteFile() {
        MockMultipartFile file = new MockMultipartFile("file", "test4.txt",
                "text/plain", "test content".getBytes());
        List<String> fileId =  s3Service.uploadFile(file);
        String result = s3Service.deleteFile(fileId.getFirst());
        assertThat(result).isNotNull();
    }
